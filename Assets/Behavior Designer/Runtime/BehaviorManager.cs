...
namespace BehaviorDesigner.Runtime
{
    public class BehaviorManager : MonoBehaviour
    {
        ...
        public void enableBehavior(Behavior behavior)
        {
            ...
            int status = addToTaskList(behaviorTree, behavior.GetBehaviorSource().RootTask, ref hasExternalBehavior, false, null);
            if (status < 0) {
        ...
        public int addToTaskList(BehaviorTree behaviorTree, Task task, ref bool hasExternalBehavior, bool fromExternalTask, Dictionary<string, object> inheritedFields)
        {
...
#pragma warning restore 0618
                ...
                if (behaviorSource != null) {
                    ...
                    if (externalRootTask != null) {
                        ...
                        int status = 0;
                        if ((status = addToTaskList(behaviorTree, externalRootTask, ref hasExternalBehavior, true, inheritedFields)) < 0) {
                            return status;
                        }
                    } else {
                        return -2;
                ...
                if (fromExternalTask) {
                    ...
                    for (int i = 0; i < fields.Length; ++i) {
                        if (inheritedFields != null && inheritedFields.ContainsKey(fields[i].Name) && fields[i].GetCustomAttributes(typeof(InheritedFieldAttribute), false).Length > 0) {
                            ...
                            fields[i].SetValue(localTask, fields[i].GetValue(task));
                        }
                    }
                ...
                if (task.GetType().IsSubclassOf(typeof(ParentTask))) {
                    ...
                    for (int i = 0; i < localParentTask.Children.Count; ++i) {
                        ...
                        if ((status = addToTaskList(behaviorTree, localParentTask.Children[i], ref hasExternalBehavior, fromExternalTask, inheritedFields)) < 0) {
                            return status;
                        }
                    }
                } else {
            ...
            for (int i = 0; i < behaviorTrees.Count; ++i) {
                ...
                for (int j = behaviorTree.activeStack.Count - 1; j > -1; --j) {
                    ...
                    if (j < behaviorTree.interruptionIndex.Count && (interruptedTask = behaviorTree.interruptionIndex[j]) != -1) {
                        ...
                        if (behaviorTree.taskList[interruptedTask] == behaviorTree.taskList[behaviorTree.activeStack[j].Peek()]) {
...
#pragma warning disable 0618
        ...
        public void interrupt(Behavior behavior, Task task)
        {
...
#if UNITY_EDITOR || DLL_DEBUG || DLL_RELEASE
                            if (behavior.logTaskChanges) {
                                Debug.Log(string.Format("{0}: {1}: Interrupt task {2} with interrupt index {3}", roundedTime(), behaviorTree.behavior.ToString(), task.GetType().ToString(), interruptionIndex));
                            }
#endif
                            break;
                        }
            ...
            if (treeObjectMap.ContainsKey(behaviorTree)) {
                ...
                var objectType = Type.GetType("BehaviorManager_PlayMaker");
                if (objectType != null) {
                ...
                if (!playMakerInvoked) {
                    objectType = Type.GetType("BehaviorManager_uScript");
                    if (objectType != null) {
        ...
        {
            return decimal.Round((decimal)Time.time, 5, MidpointRounding.AwayFromZero);
        }
